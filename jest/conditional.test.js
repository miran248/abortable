import abortable, { conditional } from "../src";
...
test("check inputs and outputs", async() => {
  ...
  const [ promise, abort ] = abortable(conditional(predicateFn, fn), 0);
  const awaiter = promise.then(resolveFn).catch(rejectFn);

  await awaiter;

  expect(predicateFn.mock.calls.length).toBe(1);
  ...
  expect(resolveFn.mock.calls[0][0]).toEqual([ true, 1 ]);
...
test("no arguments -> should reject", async() => {
  ...
  const [ promise, abort ] = abortable(conditional(), 0);
  const awaiter = promise.then(resolveFn).catch(rejectFn);
  abort();
...
test("should handle undefined operations", async() => {
  ...
  const [ promise, abort ] = abortable(conditional(predicateFn, undefined), 0);
  const awaiter = promise.then(resolveFn).catch(rejectFn);
  abort();
  ...
  expect(resolveFn.mock.calls.length).toBe(1);
  expect(resolveFn.mock.calls[0][0]).toEqual([ false, 0 ]);
  expect(rejectFn.mock.calls.length).toBe(0);
});